...
\begin{figure}[H]
  ...
  \begin{enumerate}
    \item In $K_{22}$ there are 11 total maximal length $11$ edges but $22$ of every other length.
  ...
  Now, let's begin solving $(2.)$ by just looking at $K_{21}$. How can we collect $21$ edges of length $1,2,\text{ and }3$? Well, what if we have some labelings that generate all $21$ distinct edges of lengths $8,9\text{ and }10$ between them? Naturally, we would do something similar for $K_{22}$ with edges of lengths $1,2,3,\text{ and }\infty$. Well, by imposing a new edge length on top of the standard length $\ell$, we can can achieve this. We define $\ell_{7}^{+}$ from $\ZZ_{21}\cup\{\infty\}\text{ to }\ZZ_{7}$ as follows
...
-----
  However, new lengths still come $7$ at a time at each step $t\mapsto t+1$: $K_{14t+7}$ and $K_{14t+8}$ have lengths in $\{1,\hdots 10\}\cup \cdots\cup\{7t-3,\hdots,3+7t\}$ and $\{1,\hdots 11\}\cup \cdots\cup\{7t-2,\hdots,4+7t\}$, respectively.
...
Now, we formalize the labeling described in the previous section of this chapter for $K_{n}$ where $n\equiv 8\pmod{14}$ which are of the form $K_{14t+8}$ where $t\geq 1$. 
