...
Take $V(K_{14})$ to be $\ZZ_{13}\cup \{\infty\}$, and label all edges via the length function modulo $13$ except for edges incident to $\infty$ which will refer to as length $\infty$. We do this so that intuitively, developing nodes by $1$ fix the $\infty$ vertex so that $\infty\mapsto \infty+1=\infty$. Formally:
$$\ell(uv)=\begin{cases}\min\{|u-v|,13-|u-v|\}, & u,v\neq \infty, \\ \infty, & u\text{ or }v=\infty \end{cases} \text{ and }v\mapsto 
\begin{cases}
  v+1,&v\in\mathbb{Z}_{13},\\
  \infty,        &v=\infty.
  \end{cases}$$
Another reason for doing this is that we will now have $13$ edges of lengths $1$ through $6$ as well as of length $\infty$, since the infinity node will have all $13$ edges of length $\infty$ to nodes in $\ZZ_{13}$ incident to it. Now if we develop the endpoints of an edge with any length by $1$ repeatedly, we will get all $13$ of a distinct edges of that length. So we can hopefully cyclically generate $F$-decompositions with this new construction for $K_{14}$ for each forest.
...
\noindent It is important to note that we \textbf{must} use edges $uv$ of length $|u-v|$ in $K_{13}$ so they will also have length $|u-v|$ in $K_{14}$ and $K_{15}$. If we only use edges of this type, we don't have to think about $\ZZ_{13}$ at all except when we label the vertices in the first place. We can just compute edge lengths modulo $14$. This is explained in depth in the next section. 

Putting this all together: if we can just use vertex labels from $\ZZ_{13}$ for a labeling of a forest $F$ but only use edges $uv$ of lengths $|u-v|$ modulo $14$, and then include all lengths from $1$ to $7$ in it: we can (1) simply develop the labeling by $1$ to get the $F$-decomposition of $K_{15}$, then (2) just relabel an endpoint of the length seven edge to $\infty$, and then once more develop by $1$ to get the $F$-decomposition of $K_{14}$. Note that this requires the length $7$ edge to have an endpoint with only one neighbor, as to not mess with other edge lengths. We show an example on the following page.
...
We will explain another reason edges $uv$ of lengths $|u-v|$
\subsection{Stretching a labeling}\label{sec:0,1constr-pt2}
  ...
  We show how the labeling from Figure \ref{fig:01decompexample} gives the $\mathbf{T_{7}^{11}\sqcup T_{2}^{11}}$-decomposition of $K_{14(2)}=K_{28}$ and $K_{14(2)+1}=K_{29}$.
  ...
  We will refer to this act of stretching a labeling in later sections. The ideas in this section are now put together formally in the next section.
